...
class ModernCityBotGUI:
   ...
   def create_sidebar(self):
      ...
      tk.Label(title_frame, text="CityBot", font=self.font_title, bg=self.colors['bg_secondary'], fg=self.colors['text_primary']).pack(anchor="w")
      tk.Label(title_frame, text="Assistente IA", font=self.font_small, bg=self.colors['bg_secondary'], fg=self.colors['accent']).pack(anchor="w")
      
      tk.Frame(self.sidebar_content, bg=self.colors['border'], height=2).pack(fill="x", padx=20, pady=15)
      
      menu_label = tk.Label(self.sidebar_content, text="FONTES DE DADOS", font=("Segoe UI", 9, "bold"), bg=self.colors['bg_secondary'], fg=self.colors['text_secondary'])
      ...
      tk.Label(context_container, text="📋 Contexto Atual", font=("Segoe UI", 10, "bold"), bg=self.colors['bg_secondary'], fg=self.colors['accent']).pack(anchor="w")
      self.context_frame = tk.Frame(context_container, bg=self.colors['bg_tertiary'], highlightbackground=self.colors['accent'], highlightthickness=2, bd=0)
      self.context_frame.pack(fill="x", pady=(10, 0), ipady=10)
      self.context_label = tk.Label(self.context_frame, text="Chat Livre", font=self.font_small, wraplength=240, bg=self.colors['bg_tertiary'], fg=self.colors['text_primary'])
      self.context_label.pack(anchor="w", padx=15, pady=10)
      
      self.create_menu_button("🗑️  Limpar Conversa", self.clear_chat, bg=self.colors['error']).pack(fill="x", padx=20, pady=30)
   ...
   def create_menu_button(self, text, command, bg=None):
      ...
      label = tk.Label(btn, text=text, font=self.font_menu, bg=bg_color, fg=self.colors['text_primary'], cursor="hand2", anchor="w")
      label.pack(side="left", padx=20, fill="x", expand=True)
      def on_enter(e):
         if not bg: btn.config(bg=hover_color); label.config(bg=hover_color)
      def on_leave(e):
         if not bg: btn.config(bg=bg_color); label.config(bg=bg_color)
   ...
   def create_chat_area(self):
      ...
      self.chat_canvas = tk.Canvas(self.chat_container, bg=self.colors['bg_primary'], highlightthickness=0)
      ...
      self.add_system_message("👋 Bem-vindo ao CityBot!", "Sou seu assistente inteligente. Escolha uma opção no menu lateral ou comece a conversar.")
   ...
   def create_input_area(self):
      ...
      input_container = tk.Frame(input_frame, bg=self.colors['bg_tertiary'], highlightbackground=self.colors['border'], highlightthickness=1)
      input_container.grid(row=0, column=0, sticky="ew", padx=15, pady=15)
      input_container.grid_columnconfigure(0, weight=1)
      self.input_text = tk.Text(input_container, height=3, bg=self.colors['bg_tertiary'], fg=self.colors['text_primary'], font=self.font_text, wrap="word", bd=0, highlightthickness=0, insertbackground=self.colors['accent'], padx=10, pady=10)
      ...
      def on_focus_in(event):
         if self.input_text.get("1.0", "end-1c") == "Digite sua mensagem...":
               self.input_text.delete("1.0", "end"); self.input_text.config(fg=self.colors['text_primary'])
      def on_focus_out(event):
         if not self.input_text.get("1.0", "end-1c").strip():
               self.input_text.delete("1.0", "end"); self.input_text.insert("1.0", "Digite sua mensagem..."); self.input_text.config(fg=self.colors['text_secondary'])
      self.input_text.bind("<FocusIn>", on_focus_in)
      self.input_text.bind("<FocusOut>", on_focus_out)
      send_btn = tk.Button(input_container, text="➤", font=("Segoe UI", 18, "bold"), bg=self.colors['accent'], fg=self.colors['bg_primary'], activebackground=self.colors['accent_secondary'], activeforeground=self.colors['text_primary'], bd=0, cursor="hand2", command=self.send_message, width=3, height=1)
      send_btn.grid(row=0, column=1, padx=15, pady=10)
      tk.Label(input_frame, text="💡 Enter para enviar  •  Shift+Enter para nova linha", font=self.font_small, bg=self.colors['bg_secondary'], fg=self.colors['text_secondary']).grid(row=1, column=0, sticky="w", padx=15)
      
   def create_status_bar(self):
      self.status_bar = tk.Frame(self.root, bg=self.colors['bg_secondary'], height=35)
      self.status_bar.grid(row=1, column=0, columnspan=2, sticky="ew")
      self.status_bar.grid_propagate(False)
      self.status_label = tk.Label(self.status_bar, text="●  Pronto", font=self.font_small, bg=self.colors['bg_secondary'], fg=self.colors['success'])
      self.status_label.pack(side="left", padx=25)
      self.time_label = tk.Label(self.status_bar, text="", font=self.font_small, bg=self.colors['bg_secondary'], fg=self.colors['text_secondary'])
   ...
   def pulse_animation(self):
      def animate():
         with contextlib.suppress(Exception):
            for i in range(10): scale = 1 + (i * 0.05); self.root.after(50)
   ...
   def add_message_bubble(self, text, is_user=True, animate=True):
      ...
      if is_user:
         bubble_container.grid_columnconfigure(0, weight=1); col = 1; bg_color = self.colors['accent']; fg_color = self.colors['bg_primary']; anchor = "e"
      else:
   ...
   def animate_typing(self, label, text, index=0):
      if index < len(text):
         label.config(text=text[:index+1]); self.root.after(15, lambda: self.animate_typing(label, text, index+1))
         
   def add_system_message(self, title, message):
      container = tk.Frame(self.messages_frame, bg=self.colors['bg_primary'])
      container.pack(fill="x", padx=30, pady=30); container.grid_columnconfigure(0, weight=1)
      inner = tk.Frame(container, bg=self.colors['bg_secondary'], padx=40, pady=25); inner.grid(row=0, column=0)
   ...
   def add_loading_indicator(self):
      self.loading_frame = tk.Frame(self.messages_frame, bg=self.colors['bg_primary']); self.loading_frame.pack(fill="x", padx=30, pady=15)
      dots = tk.Label(self.loading_frame, text="● ● ●", font=("Segoe UI", 24, "bold"), bg=self.colors['bg_primary'], fg=self.colors['accent']); dots.pack()
      def animate_dots(count=0):
         if hasattr(self, 'loading_frame') and self.loading_frame.winfo_exists():
               ...
               dots.config(fg=colors[0]); self.root.after(400, lambda: animate_dots(count + 1))
      animate_dots(); self.messages_frame.update_idletasks(); self.chat_canvas.yview_moveto(1.0)
      
   def remove_loading_indicator(self):
      if hasattr(self, 'loading_frame'): self.loading_frame.destroy(); delattr(self, 'loading_frame')
   ...
   def send_message(self):
      ...
      self.input_text.delete("1.0", "end"); self.input_text.insert("1.0", "Digite sua mensagem..."); self.input_text.config(fg=self.colors['text_secondary'])
      self.add_message_bubble(message, is_user=True); self.is_processing = True
      self.status_label.config(text="●  Processando...", fg=self.colors['warning']); self.add_loading_indicator()
      thread = threading.Thread(target=self.process_message, args=(message,)); thread.daemon = True; thread.start()
   ...
   def show_error(self, error_msg):
      self.remove_loading_indicator(); self.add_message_bubble(f"❌ Erro: {error_msg}", is_user=False, animate=False)
      self.is_processing = False; self.status_label.config(text="●  Erro", fg=self.colors['error'])
   ...
   def show_input_dialog(self, title, prompt, callback):
      ...
      entry = tk.Entry(dialog, font=self.font_text, width=45, bg=self.colors['bg_tertiary'], fg=self.colors['text_primary'], insertbackground=self.colors['accent'], bd=0, highlightthickness=1, highlightbackground=self.colors['border']); entry.pack(pady=10, ipady=8); entry.focus()
      def on_ok(): value = entry.get(); dialog.destroy(); callback(value)
      btn_frame = tk.Frame(dialog, bg=self.colors['bg_secondary']); btn_frame.pack(pady=25)
...
if __name__ == "__main__":
   root = tk.Tk(); app = ModernCityBotGUI(root); root.mainloop()
